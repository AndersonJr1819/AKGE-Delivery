document.addEventListener('DOMContentLoaded', () => {
    ...
    function getAIResponse(userText) {
        ...
        if (query.includes('guioza') || query.includes('acompanhamento')) {
            return getRandomResponse([
                "Para acompanhar seu combinado, temos a porção de Guioza dourada e recheada por R$ 18,00. 🥟"
            ]);
        }

        if (query.includes('bebida') || query.includes('chá') || query.includes('cha')) {
            return getRandomResponse([
                "Temos Chá Gelado refrescante por R$ 7,00 para acompanhar sua refeição. 🍵"
            ]);
        }

        if (query.includes('doce') || query.includes('sobremesa') || query.includes('mochi')) {
            return getRandomResponse([
                "Para a sobremesa, temos o tradicional Mochi com massa macia e recheio cremoso por R$ 14,00. 🍡"
            ]);
        }
