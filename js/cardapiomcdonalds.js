document.addEventListener('DOMContentLoaded', () => {
    ...
    function getAIResponse(userText) {
        ...
        if (query.includes('batata') || query.includes('acompanhamento')) {
            return getRandomResponse([
                "Para acompanhar seu lanche, temos a Batata Frita crocante por R$ 10,00. 🍟"
            ]);
        }

        if (query.includes('bebida') || query.includes('refrigerante') || query.includes('drink')) {
            return getRandomResponse([
                "Temos Refrigerante gelado por R$ 6,00 para acompanhar seu pedido. 🥤"
            ]);
        }

        if (query.includes('doce') || query.includes('sobremesa') || query.includes('brownie')) {
            return getRandomResponse([
                "Para a sobremesa, temos o delicioso Brownie de chocolate por R$ 12,00. 🍫"
            ]);
        }
